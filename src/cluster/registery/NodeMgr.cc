...
void NodeMgr::RegisterNode(util::other::Uuid uuid, const NodeRegInfo& info)
{
...
void NodeMgr::NodeOffline(bbt::network::ConnId connid)
{
    ...
    if (it != m_connid_map.end()) {
        auto& node = m_node_map[it->second];
        node.SetStatus(NodeState::NODESTATE_OFFLINE);
        node.SetConnId(0);
    }
...
NodeRegInfo* NodeMgr::GetNodeInfo(util::other::Uuid uuid)
{
...
NodeRegInfo* NodeMgr::GetNodeInfo(bbt::network::ConnId connid)
{
    auto it = m_connid_map.find(connid);
    if (it == m_connid_map.end())
        return nullptr;
