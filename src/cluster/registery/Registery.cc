...
util::errcode::ErrOpt Registery::SendToNode(emR2NProtocolType type, const util::other::Uuid& uuid, const bbt::core::Buffer& proto)
{
    ...
    head.protocol_length = sizeof(R2NProtocolHead) + proto.Size();
...
util::errcode::ErrOpt Registery::OnHeartBeat(bbt::network::ConnId id, N2R_KeepAlive_Req* req)
{
    ...
    return SendToNode(R2N_KEEPALIVE_RESP, uuid, bbt::core::Buffer{(char*)&resp, sizeof(resp)});
}
...
util::errcode::ErrOpt Registery::OnGetNodesInfo(bbt::network::ConnId id, N2R_GetNodesInfo_Req* req)
{
    ...
    for (auto&& node : nodes) {
        ...
        nodeinfo.set_uuid(node->GetUuid().ToString());
